...
export function ProfileAction({
    ...
    async function saveProfile() {
        ...
        if (nextBannerImage !== (initialBannerImage ?? ""))
            payload.bannerImage = nextBannerImage || null
        if (bio !== (initialBio ?? "")) payload.bio = bio || null
        if (pronoun !== (initialPronoun ?? ""))
            payload.pronoun = pronoun || null
        if (location !== (initialLocation ?? ""))
            payload.location = location || null
        if (website !== (initialWebsite ?? ""))
    ...
    if (currentUserId === userId) {
        return (
            <Dialog open={editOpen} onOpenChange={setEditOpen}>
                ...
                <DialogContent
                    className="max-h-[90vh] w-full !max-w-lg overflow-y-auto border-2 border-border bg-card"
                ...
                >
                    ...
                    <p className="text-sm text-muted-foreground">
                        this is a place for you to express yourself through your
                        public profile! please keep it appropriate.
                    </p>
    ...
    function toggleFollow() {
        ...
        startTransition(async () => {
            try {
                ...
                // cool toaster thing
                toast.success(
                    `You ${res.following ? "followed" : "unfollowed"} ${username}`
                )
            } catch {
