            ...
            <WidgetContent>
                ...
                <div className="mt-3 space-y-3">
                    {hashtags.length === 0 ? (
                        ...
                        <Link key={hashtag.tag} href={`/trending/${hashtag.tag}`} className="flex items-center justify-between hover:underline">
                            <span className="font-semibold text-primary-2">#{hashtag.tag}</span>
                            <span className="text-sm text-muted-foreground">{hashtag.count.toLocaleString()}</span>
                        </Link>
                    ))}
                </div>
